    ...
    server.registerTool("check_message_privacy", {
        ...
        inputSchema: {
            ...
            context: z
                ...
                .optional()
                .default("general")
                .describe("Message context that adjusts risk weight: messenger (personal chat), email, notice (public announcement), work (business message), general (default)"),
            strictness: z
            ...
            showMasked: z
                ...
                .optional()
                .default(true)
                .describe("Whether to include masked and safe-rewrite versions in the response"),
        },
        annotations: {
            title: "Check Message Privacy",
            readOnlyHint: true,
        ...
        try {
            ...
            if (result.detectedItems.length > 0) {
                ...
                for (const item of result.detectedItems) {
                    lines.push(``, `**[${LEVEL_EMOJI[item.level]} ${item.type}]** (${item.count} match${item.count > 1 ? "es" : ""})`);
