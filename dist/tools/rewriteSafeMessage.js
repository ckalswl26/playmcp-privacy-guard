    ...
    server.registerTool("rewrite_safe_message", {
        ...
        try {
            ...
            if (result.detectedItems.length === 0) {
                lines.push(`✅ No personal information detected. The original message is safe to send.`);
            }
            else {
                lines.push(`### Detected PII`);
                for (const item of result.detectedItems) {
                    lines.push(`- ${LEVEL_EMOJI[item.level]} **${item.type}** (${item.level}): ${item.description}`);
                ...
                lines.push("```");
                lines.push(``, `**Option 2 — Category Tags** ✅ Recommended (fully removed)`);
                lines.push("```");
                ...
                lines.push(``, `> If sharing personal information is essential, use a secure channel (direct call, encrypted email) instead of messaging apps.`);
            }
