    ...
    server.registerTool("mask_sensitive_info", {
        ...
        inputSchema: {
            ...
            maskStyle: z
                ...
                .optional()
                .default("star")
                .describe("Masking style: star (010-****-1234, partial form visible) or tag ([Phone number omitted], fully removed)"),
            context: z
                .enum(["messenger", "email", "notice", "work", "general"])
                .optional()
                .default("general")
                .describe("Message context that adjusts risk weight: messenger, email, notice, work, general"),
        },
        annotations: {
            title: "Mask Sensitive Info",
            readOnlyHint: true,
        ...
        try {
            ...
            const output = maskStyle === "tag" ? result.safeRewrite : result.maskedMessage;
            ...
            if (result.detectedItems.length > 0) {
                ...
                for (const item of result.detectedItems) {
                    lines.push(`- ${LEVEL_EMOJI[item.level]} ${item.type} (${item.level}, ${item.count} match${item.count > 1 ? "es" : ""}): ${item.description}`);
                }
            }
