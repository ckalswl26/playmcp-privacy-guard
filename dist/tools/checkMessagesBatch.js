    ...
    server.registerTool("check_messages_batch", {
        ...
        inputSchema: {
            ...
            strictness: z
                ...
                .optional()
                .default("normal")
                .describe("Detection strictness: strict (×1.5 score), normal (×1.0), lenient (×0.7)"),
        },
        annotations: {
            title: "Check Messages Batch",
            readOnlyHint: true,
        ...
        try {
            ...
            for (let i = 0; i < messages.length; i++) {
                ...
                if (!result.safeToSend)
                    dangerCount++;
                const maskedPreview = result.maskedMessage.length > 50
                    ? result.maskedMessage.slice(0, 50) + "…"
                    : result.maskedMessage;
                lines.push(`### Message ${i + 1} ${LEVEL_EMOJI[result.overallRisk]} — ${result.safeToSend ? "Safe" : "**Review required**"}`);
                lines.push(`> "${maskedPreview}"`);
                lines.push(`- Risk: ${result.overallRisk} (score: ${result.riskScore}/100)`);
                if (result.detectedItems.length > 0) {
                    lines.push(`- Detected: ${result.detectedItems
            ...
            lines.push(`---`);
            lines.push(`**${dangerCount} of ${messages.length} message${messages.length > 1 ? "s" : ""} require review** (${messages.length - dangerCount} safe)`);
